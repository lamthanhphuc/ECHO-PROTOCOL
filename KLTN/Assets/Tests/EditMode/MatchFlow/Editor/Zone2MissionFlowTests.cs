...
namespace EchoProtocol.Tests.MatchFlow
{
    ...
    public class Zone2MissionFlowTests
    {
        ...
        public void SetUp()
        {
            ...
            termGo.AddComponent<BoxCollider>();
        ...
        public void TEST_19_Offline_Director_Bridges_MatchFlow_To_FinalHunt()
        {
            ...
            Assert.IsTrue(
                _director.IsSecurityHoldComplete);

            Assert.AreEqual(
                Zone2MissionStage.AuthorizationCodeGranted,
                _director.CurrentStage);

            Assert.AreEqual(
                MatchPhase.PowerPuzzle,
                flow.Phase,
                "Offline Security Hold completion must advance MatchFlow to PowerPuzzle.");

            Assert.IsTrue(
                flow.IsSecurityHoldComplete);

            Assert.AreEqual(
                _director.AuthorizationCode,
                flow.PowerAuthorizationCode,
                "Zone2MissionDirector and MatchFlowController must use the same authorization code.");

            bool accepted =
                _director.SubmitAccessCode(
                    _director.AuthorizationCode);
