...
export default async function WalletPage() {
  ...
  return <>
    ...
    <Panel className="overflow-hidden">
      ...
      <p className="mt-3 text-4xl font-bold text-white">{formatNumber(profile.walletBalance)} <span className="text-lg text-emerald-300">Coins</span></p>
    </Panel>
    <Panel className="mt-5">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {catalog.items.map((item) => <article key={item.productReference} className="rounded-lg border border-[#1d3942] bg-[#071014] p-5">
          <p className="eyebrow">{item.provider}</p>
          <h3 className="mt-3 text-xl font-semibold text-white">{item.displayName}</h3>
          <p className="mt-3 text-3xl font-bold text-emerald-300">{formatNumber(item.walletCredit)} <span className="text-sm">Coins</span></p>
